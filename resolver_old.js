...
function Resolver(jobs) {
    ...
    for (let i = 0; i < jobs.length; i++) {
        ...
        job.after       = toObject(job.after);

        // Provide self
        job.provides[job.name] = true;

        // NotIf implies a specific execution order.
    ...
    for (let i = 0; i < tree.jobs.length; i++) {
        ...
        // Look if we have reverse dependencies and add them to any found jobs as 'after'.
        for (let before in job.before) {
            for (let k = 0; k < tree.jobs.length; k++) {
                if ((tree.jobs[k].name === before) || (tree.jobs[k].provides[before] === true)) {
                    tree.jobs[k].after[job.name] = true;
                }
            }
        }
